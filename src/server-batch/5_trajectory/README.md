...
Builds `web/ephemeris/itinerary.json` — mission event timeline consumed by
the trajectory visualization UI for event tick marks and the Last/Next event
HUD display.
...
4. Add an events list in `_EVENTS` inside `5d_web_itinerary.py`.
5. Run 5a → 5b → 5c → 5d.
